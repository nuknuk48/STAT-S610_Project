...
- Sample the parameter $\theta^*$ from prior distribution
- Simulate the dataset $D^*$ using distribution $f(D^*|\theta^*)$
- Compared the simulated dataset $D^*$ with the observed dataset $D_0$ using the distance function $d$ and tolerance $\epsilon$ to accept or discard the parameter $\theta^*$
- Run this process multiple times to get a posterior distribution for parameter $\theta^*$ 
...
- Use the equations on Page 107 from Toni and Stumpf's papaer as criteria to simulate datasets
...
The Distance function uses a Frobenious norm, which we imported from the matrixcalc package. 
...
Distance <- function(data1, data2, generated_data) {
  ...
  return((1/2) * (frobenius.norm(data1 - generated_data$w1) + 
                    frobenius.norm(data2 - generated_data$w2)))
}
```
...
We tried several epsilon values and decided on an epsilon value of 0.5 being the most favorable. 
